...
def _incremental_output_download(
    ...
    try:
        ...
        _pid_utils.check_and_obtain_pid_lock_if_available(download_progress_location, logger)
    except RuntimeError as e:
        logger.echo(f"Download failed because of error : {e}")
        return
    except Exception as e:
        logger.echo(
            f"Failed to obtain lock for download progress at {download_progress_location} due to unexpected exception : {e}"
        )
        return


def _validate_file_inputs_for_incremental_output_download(
    download_progress_location: str, path_mapping_rules: Optional[str] = None
):
    """
    ...
    if not os.path.isdir(download_progress_location):
        raise RuntimeError(
            f"Download progress location {download_progress_location} is not a valid directory"
        )

    # Check that download progress location is writable
    if not os.access(download_progress_location, os.W_OK):
        raise RuntimeError(
            f"Download progress location {download_progress_location} exists but is not writable, please provide write permissions"
        )
